import type { QuizOption, QuizQuestion, ResultDefinition, ResultKey, ScoreMap } from "./quiz-types";

export const resultMap: Record<ResultKey, ResultDefinition> = {
  info: {
    label: "信息缺口型",
...
export const concernCopy: Record<string, string> = {
  student: "你最近最烦的是什么？",
...
function toOption(option: readonly [string, string, string, Partial<ScoreMap>] | unknown[]): QuizOption {
  const [value, label, description, scores] = option as [string, string, string, Partial<ScoreMap>];
...
export const questions: QuizQuestion[] = [
  ...
  },
  {
    id: "concern",
    kicker: "近期苦恼",
    title: ({ identity }: { identity?: string }) => concernCopy[identity || ""] || "你最近最烦的是什么？",
    progress: "定位近期苦恼",
    options: [
      {
        value: "unknown",
